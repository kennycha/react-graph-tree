...
const CanvasContent = styled.div<{ $zoom: number; $offset: Position }>`
  width: 100%;
  height: 100%;
  transform: scale(${props => props.$zoom}) translate(${props => props.$offset.x}px, ${props => props.$offset.y}px);
...
const GridBackground = styled.div<{ $zoom: number; $offset: Position }>`
  ...
  background-position: ${props => props.$offset.x * props.$zoom}px ${props => props.$offset.y * props.$zoom}px;
  opacity: 0.3;
`;
...
export const GraphCanvas: React.FC<GraphCanvasProps> = ({ children }) => {
  const containerRef = useRef<HTMLDivElement>(null);
  const [isPanning, setIsPanning] = useState(false);
  const [panStart, setPanStart] = useState<Position>({ x: 0, y: 0 });
  
  const { graph, updateViewState } = useGraphStore();
  const { zoom, offset } = graph.viewState;
  ...
  const handleWheel = useCallback((e: WheelEvent) => {
    ...
    const newZoom = Math.max(0.25, Math.min(2.0, zoom * zoomFactor));
